...
<head>
	...
	<script type="text/javascript">
		...
		var student_by_class = <?php echo json_encode($student_by_class) ?> 
		...
		function check(){
			...
			if(ma_sinh_vien == ''){
				...
				for(i = 0; i < student_by_class.length ; i++){

					if(ma_sinh_vien == student_by_class[i]['ma_sinh_vien']){
				...
				if(flag != 1){
					...
					for(i = 0; i < all_student.length; i++){
						...
						if(ma_sinh_vien == all_student[i]['ma_sinh_vien']){
							...
							document.getElementById("ho_ten").value = all_student[i]['ho_ten'];
			...
			if(flag == 0 || flag == 3){
				...
				if(flag == 1){
					...
					infor.innerHTML = "<button class='btn' type='submit'>Mã SV đã có trong lớp</button>";
...
<body>
	<div class="container" >
  		...
  		<h2>Thêm sinh viên vào lớp <?php echo $class; ?></h2>
  		<form class="form" action="insert_student_by_class" method="get">
  			<input type="hidden" name="id_class" value ="<?php echo $id_class ?>"/>
    
    	<fieldset class="form-fieldset ui-input __first">
      		<input type="text" id="ma_sinh_vien" tabindex="0" onkeyup="check()" name="ma_sinh_vien"/>
    	...
    	<fieldset class="form-fieldset ui-input __second">
      		<input type="text" id="ho_ten" tabindex="0" onkeyup="check()" name="ho_ten"/>
      		<label for="ho_ten">
